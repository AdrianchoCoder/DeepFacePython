...
def analyze_image(image_path: str | Path) -> list[dict]:
    ...
    for r in results:
        ...
        face_data = {
            ...
            "emocion": EMOTION_LABELS_ES.get(
                dominant_emotion.lower(), dominant_emotion.capitalize()
            ),
            "emocion_key": dominant_emotion.lower(),
            "emocion_emoji": EMOTION_EMOJI.get(dominant_emotion.lower(), "😐"),
            "emociones_detalle": [
                {
                    "nombre": EMOTION_LABELS_ES.get(k.lower(), k),
                    "porcentaje": round(float(v), 1),
                }
                for k, v in r_clean.get("emotion", {}).items()
            ],
            "razas_detalle": {
                RACE_LABELS_ES.get(k, k): round(float(v), 1)
...
def draw_annotations(img: np.ndarray, faces: list[dict]) -> np.ndarray:
    """Dibuja rectángulos cian alrededor de los rostros detectados."""
    cyan = (255, 255, 0)  # BGR
    for i, face in enumerate(faces, start=1):
        ...
        cv2.rectangle(img, (x, y), (x + w, y + h), cyan, 3)
        label = f"Rostro {i}"
        (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)
        cv2.rectangle(img, (x, y - th - 14), (x + tw + 8, y), cyan, -1)
        cv2.putText(
            img,
            label,
            (x + 4, y - 8),
